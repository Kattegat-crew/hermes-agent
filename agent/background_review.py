...
_SKILL_REVIEW_PROMPT = (
    "Review the conversation above and execute the autonomous 4-way evaluation for the skill library.\n\n"
    "EVALUATION PROTOCOL (Strict Order of Operations):\n\n"
    "Step 1: NOVELTY & DURABILITY FILTER (Discard / Nothing to Save)\n"
    "  • Ask: Did this session discover a genuinely novel, non-trivial, verified, and repeatable technical procedure or pattern that future sessions will need?\n"
    "  • If the session was routine conversation, standard Q&A, simple tasks, minor or transient errors already resolved, or unverified attempts: reply 'Nothing to save.' and STOP immediately.\n"
    "  • 'Nothing to save.' is the DEFAULT, EXPECTED outcome for routine sessions. Do NOT force or invent skill updates.\n\n"
    "Step 2: NUTRIR / ENRICH EXISTING SKILL (Preferred Action)\n"
    "  • If a durable technique, workaround, user workflow correction, or pitfall was discovered:\n"
    "    1. Check currently loaded skills and search the library (`skills_list`, `skill_view`) to see if an existing skill or umbrella covers this technical domain (e.g. docker-management, vault-access, hermes-fleet-ops, etc.).\n"
    "    2. If a matching or related skill exists: DO NOT CREATE A NEW SKILL. NUTRIR (enrich) the existing skill instead!\n"
    "    3. Patch SKILL.md via `skill_manage(action='patch', name=..., old_string=..., new_string=...)` to add instructions, pitfalls, or commands (remember to call `skill_view(name)` first for read-before-write).\n"
    "    4. Or add supporting files via `skill_manage(action='write_file', name=..., file_path='references/<topic>.md', file_content=...)` for session notes, recipes, or documentation, or `scripts/<name>.<ext>` for runnable tools.\n"
    "  • Canonical fleet skills in the library CAN and SHOULD be enriched autonomously. This preserves a compact, high-quality library.\n\n"
    "Step 3: CREATE A NEW SKILL ONLY FOR GENUINELY NEW DOMAINS\n"
    "  • ONLY create a new skill (`skill_manage(action='create', ...)`) if NO existing skill or umbrella in the library encompasses this domain.\n"
    "  • The skill MUST be class-level / domain-level (e.g. 'shopify-inventory-sync', NOT 'fix-bug-today' or session artifacts).\n"
    "  • Must include valid YAML frontmatter with 'name' and 'description'.\n"
    "  • Created directly in the canonical library autonomously without quarantine and without requiring human signature.\n\n"
    "Step 4: DELETION GUARD\n"
    "  • Never delete existing canonical skills autonomously. Deletion is restricted.\n\n"
    "Read-before-write (ENFORCED): before patching or editing an existing skill's SKILL.md, call `skill_view(name)` during this review. Before overwriting an existing supporting file, call `skill_view(name, file_path=...)`.\n\n"
    "Protected skills (DO NOT edit these):\n"
    "  • Bundled skills shipped with Hermes (e.g. 'hermes-agent').\n"
    "  • Hub-installed skills.\n"
    "  • PINNED skills (marked via 'hermes curator pin').\n\n"
    "Do NOT capture:\n"
    "  • Environment-dependent or transient errors ('command not found', missing package before apt install).\n"
    "  • Negative claims about tools ('tool X does not work').\n"
    "  • Unresolved failures or dead ends.\n\n"
    "If nothing genuinely novel was discovered, say 'Nothing to save.' and stop."
)

_COMBINED_REVIEW_PROMPT = (
    "Review the conversation above and update two things:\n\n"
    "**Memory**: who the user is. Save facts about the user, persona, desires, and durable preferences with the memory tool.\n\n"
    "**Skills**: how to do this class of task. Execute the autonomous 4-way evaluation:\n"
    "  1. Novelty Filter (Discard): If the turn is routine, standard, transient, or unverified, do NOT create or edit skills. 'Nothing to save.' is the default for routine sessions.\n"
    "  2. Nutrir / Enrich Existing (Preferred): If a durable technique, pitfall, or workflow correction emerged, search existing skills (`skills_list`, `skill_view`). If any related skill exists, enrich it via `action='patch'` or `action='write_file'` (references/ or scripts/). DO NOT create a new skill when an existing umbrella fits.\n"
    "  3. Create Only for New Domains: If and only if NO existing skill represents the domain, create a new class-level skill via `action='create'`. Created directly in the canonical library, zero quarantine, zero signature.\n"
    "  4. Autonomous Execution: Execute the required patch, write_file, or create directly. Never delete canonical skills.\n\n"
    "Read-before-write (ENFORCED): call `skill_view(name)` before patching SKILL.md.\n"
    "Protected skills: bundled skills, hub skills, and pinned skills are off-limits.\n"
    "Do not capture transient errors, environment mishaps, negative tool claims, or unresolved failures.\n\n"
    "Act on whichever dimension has real signal. If neither has durable novel signal, say 'Nothing to save.' and stop."
)
