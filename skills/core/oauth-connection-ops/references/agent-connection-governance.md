...
> **Alcance (17-sep-2026):** la Capa 5 (`ap_call`) es para el acceso MULTI-TENANT
> controlado (clientes/agentes por conexión), NO para el MCP propio de la flota.
> El acceso de Ragnar/roshi a AP va por el par MCP `ap`/`ap_write` (escritura con
> confirmación por llamada) — ver skill `activepieces-mcp-ops`. **No borrar
> `ap_call`:** es el enforcement documentado y referenciado por estas skills.
