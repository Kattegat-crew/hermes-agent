...
- **Si el panel no aparece, deja de pelear con las zonas**: la entrada GARANTIZADA
  es una **página propia** + **fila en la barra lateral** — superficies de primera
  clase que no dependen de zonas ni de barras colapsadas:

  ```javascript
  ctx.register({ id: 'page', area: ROUTES_AREA, data: { path: '/<id>' }, render: () => jsx(Page, {}) })
  ctx.register({ id: 'nav', area: SIDEBAR_NAV_AREA, data: { path: '/<id>', label: '<Nombre>', codicon: 'pulse' } })
  ```

  Y **`host.notify({ kind: 'info', message: 'vX cargado' })` en `register()`** como
  PRUEBA DE CARGA: si el toast no sale al recargar, el fallo es el archivo
  (ruta/nombre `plugin.js` vs `.js.txt`/reload); si sale y la fila no está, es la
  versión del app. Confirmado el 11-sep-2026: en el Desktop del Admin el chip y
  `placement: 'right'` funcionaban a medias y el panel con `dock` **nunca**
  apareció; la página + fila sí, a la primera. Regla práctica: registrar SIEMPRE
  chip + página/fila, y el panel de zona como extra, no como única vía.
- **`placement: 'right'` NO se ve por defecto**: en las releases actuales cae en
  la barra lateral derecha (la del file browser), que viene **colapsada** por
  defecto — el usuario reporta "solo me aparece el chip". Para que el panel se vea
  de entrada: `data: { placement: 'bottom', dock: { pane: 'workspace', pos: 'bottom' }, height: '200px' }`
  (receta literal de la SDK: "below the conversation"). Alternativa sin
  reinstalar: el usuario abre esa barra con **mod+J**
  (`view.toggleRightSidebar`).
- **Caso verificado (11-sep-2026, DEV)**: proceso que servía al Desktop =
  `hermes dashboard --port 9112` (PPID 1, 12 h de vida) con las conexiones del
  cliente; el `serve --isolated` del modo SSH existía con **cero** conexiones.
  Tras el SIGTERM, la app lo levantó de nuevo sola (PID nuevo, ~30 min después en
  la práctica) y el arranque dejó `Mounted plugin API routes: /api/plugins/nan-usage/`.
  Prueba de que la UI pedía datos en vivo: el mtime de `state/nan-quota.json`
  cambió a los 2 min del arranque (el endpoint consulta a NaN cuando su caché
  pasa de 120 s). No prometer reinicio instantáneo: verificar, no esperar.
