...
## 5b. Publish-as-is variant (no overlay requested)

"Publica este video en la story de golden" with no text instruction = publish the source untouched except for the
canvas pass: 576x1024 is already exact 9:16, so `scale=1080:1920` needs **no crop**, keep `-c:a copy` (music), burn
no text. Do not ask for approval on an edit that does not exist — publish, then prove it with Meta's own
`thumbnail_url`. Golden Game has ONE IG account (`@goldengame_casinos`, `instagram_demal-molala`); the sede
(Agua de Dios, Cachipay…) is context for the copy, not a separate account, and the API cannot add location
stickers or mentions to a story — that has to be done by hand in the app.
